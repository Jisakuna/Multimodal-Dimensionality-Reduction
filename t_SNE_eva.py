...
from project_paths import TSNE_OUTPUT_DIR

# 📁 设置降维后特征所在目录（每个文件为一种焊接类型）
input_dir = TSNE_OUTPUT_DIR
...
for filename in os.listdir(input_dir):
    if filename.endswith(".npy"):
        ...
        data = np.load(path)
        label = os.path.splitext(filename)[0]  # 从文件名中提取类别名
        X_all.append(data)
        y_all.extend([label] * len(data))  # 为每一行样本分配类别标签
